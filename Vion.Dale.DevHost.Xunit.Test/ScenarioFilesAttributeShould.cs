...
namespace Vion.Dale.DevHost.Xunit.Test
{
    ...
    public class ScenarioFilesAttributeShould
    {
        ...
        public async Task Discover_every_committed_scenario_as_an_id_topology_row()
        {
            ...
            CollectionAssert.AreEquivalent(new[] { "showcase-tour", "io-control", "grid-demand", "plant-control", "minimal-subset" }, byId.Keys.ToList());
        ...
        public async Task Filter_to_a_single_topology_when_requested()
        {
            ...
            CollectionAssert.AreEquivalent(new[] { "showcase-tour", "io-control", "grid-demand", "plant-control" }, defaultRows.Select(r => (string)r.GetData()[0]!).ToList());
