...
**A stale tool says so.** `CommandHelpers.RequireProject` cautions when the running tool's release is older than the `Vion.Dale.Sdk` version the resolved project references, and names `dotnet tool update`. Only that direction — a newer tool against a pinned older SDK is a deliberate pin — and never for a `0.0.0` build on either side, which is on no feed and orders against nothing. It exists because a stale global tool renders the previous release's view of a project that has moved on with nothing saying which half is old; one post-release check read that as a failed release. The tool's own version is the fourth seam in `CliComposition`, which is what lets the caution be tested at all: a local build reports `0.0.0-local` and is silent by design.
