...
namespace Vion.Dale.Cli.Infrastructure
{
    ...
    ///     transports, its credential-store root and the version it reports itself as are process-wide, and
    ///     this is the one place that sets them. Production goes through the same four seams a test
    ///     overrides, so neither is a private path the other cannot take — a test overrides a value
    ///     production also sets, rather than filling a hole production leaves empty.
    /// </summary>
    internal static class CliComposition
    {
        /// <summary>
        ///     The real transport for both HTTP clients, the real user profile for the credential store, and
        ///     the running assembly's own version.
        ...
        internal static void UseProductionDependencies()
        {
            ...
            CommandHelpers.UseToolVersion(Program.Version());
        }
    }
}