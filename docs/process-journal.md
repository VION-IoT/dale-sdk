...
2026-09-10 · review · T-019 · The review round found five blockers in the two consumer-facing pages, and every one was a claim carried from a source doc instead of read off the tree: `DALE012` and `DALE013` published as Error where the descriptors say Warning (`DaleDiagnostics.cs:129,187`, no `effectiveSeverity` overload); a `ScenarioRunRegistry` constructor argument that never existed in any shipped version (`git show v0.11.2:…` — the class has no constructor, then or now); "the first two were already `DALE043` at build time" when there is no registration analyzer at all, so that refusal is new at pack for a library that builds perfectly clean; the Modbus zero-`count` history inverted (only zero `quantity` is new, and the migration page had it right while the notes had it wrong, so the two pages disagreed); and a dual-annotated member count of 14 that reproduces at neither the pass's tree nor today's. Each was a relay section quoted as if it were the code. A relay section is one pass's summary written at its own moment, and a release page is read as current truth — the two need the same verification a criterion citation needs, one grep each. (D10, P1, P4) (escape)
