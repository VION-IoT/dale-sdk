...
2. *(c — propose-and-wait, `T-008`)* The public-API ratchet's scope: the two DevHost packages,
   ...
   **Corrected by `T-006` and `T-008`:** five entries are six, "four undeclared SDK namespaces" is
   seventeen, and the set named here is 389 unmarked public types across **eight** packages, two of
...
- `T-008` *(Opus, high)* — **the public-API ratchet covers everything shipped** (question 2:
  ...
  *(Landed as decision 0145 plus the Modbus half — two of the six entries closed. The other four
  are 348 unmarked public types across five packages and stay in the ledger with their counts and
...
the Modbus half resolved, then 72 when arming the analyzer found one** — the checker reports
`72 entry(ies) … 77 row(s), 5 resolved`). Both new rows are marked as having entered after the
...
| `LIFE` | `Vion.Dale.ProtoActor`, and three namespaces of `Vion.Dale.Sdk`, are outside the public-API snapshot. | decision | `T-008` — `Vion.Dale.ProtoActor` and the undeclared SDK namespaces; decided by decision 0145 and left here with its measured size (12 types across two packages, plus 47 of the SDK's 221). |
...
| `BIND` | Seventeen namespaces of `Vion.Dale.Sdk` are outside the public-API ratchet, not four. | decision | `T-008` — the undeclared SDK namespaces; decided by decision 0145 and left here at 221 unmarked types, of which 97 under `Examples.*` the decision rules out of scope. The lead was renamed to carry the count. |
...
- **`T-008`: the ratchet's scope is 389 unmarked public types across eight packages, and the task is
  the Modbus eighth of it.** Reviewer's question 2 named "the two DevHost packages,
  ...
  *unmarked* types has to come from the assemblies, which is where a source count and an assembly walk
  disagree by tens: `Vion.Dale.Sdk` **221** across 17 undeclared namespaces (97 of them `Examples.*`),
  `Vion.Dale.DevHost` **98** across 5, `Vion.Dale.DevHost.Web` **14** across 6, Modbus Core/Rtu/Tcp
  **41**, `Vion.Dale.ProtoActor` **10**, `Vion.Dale.DevHost.Xunit` **3**, `Vion.Dale.Plugin` **2**.
  Every one of the 389 is a per-type judgment, and each one answered `[PublicApi]` also owes a
  docs-site XML summary, because a `[PublicApi]` type's docs are a shipping surface. That is seven
  package-sized passes, not one task. The ruling does not move — decision **0145** records it for the
  whole roster — and `T-008` lands the half the ledger says is one piece of work (`TKIT`'s row **is**
  the Modbus half) and leaves the other four entries in the ledger with their measured numbers and
  the decision cited. Two shipped packages nobody had named are in the same position:
