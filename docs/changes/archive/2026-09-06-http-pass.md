...
marked), 25 public members and 143 test cases, every one citing an id. The pass states what a block author registers, calls, receives back on the actor and sees
...
`[assembly: DaleSharedAssembly]`, and **65 test methods / 143 cases, every one citing an id**
(`grep -rho "\[TestMethod\]" Vion.Dale.Sdk.Http.Test/*.cs | wc -l` → 65;
`grep -rho 'TestProperty("spec", "AC-HTTP' Vion.Dale.Sdk.Http.Test/ | wc -l` → 65;
`dotnet test Vion.Dale.Sdk.Http.Test --no-restore` → `Passed! - Failed: 0, Passed: 143, Skipped: 0,
Total: 143`). The executor's **four** `try` blocks became **five** — the fourth was
`TryInvokeCallback`'s, and the fifth is the dispose-and-rethrow inside `SendAsync` that row 42's fix
added. *(This paragraph was measured at the pass's REPORT and re-measured by the fix-up round, whose
six items added six test methods and nine cases and left the type and member counts alone — the
