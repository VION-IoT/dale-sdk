...
33 tests and **zero** citations; as the pass and its fix-up round leave it, 6 public types (all
marked), 25 public members and 144 test cases, every one citing an id. The pass states what a block author registers, calls, receives back on the actor and sees
...
1. **Delegation.** The Phase A classification (`amend-http-pass-1.md`, with its second opinion
   `http-pass-phaseA-second-opinion.md` — it re-ran the pass's probes A–AB in a scratch worktree,
   wrote fifteen more, and opened the readers: the Energy example's two `GetJson` sites, the first
   consumer's evaluation note and its zero source usage, the private runtime, the CLI's own client,
   the five cited spec pages, the manifest generator, the `DaleSharedAssembly` precedent) and round
   2's brief (`brief-http-fixup-dale-sdk.md`) were made by the coordinator (`sdk: sdd pass 7`) under
   the operator's standing delegation ("decide with best intentions, list what you decided before I
   merge"); every decision is on the coordinator's list for the operator's review at the PR. Ratified
   here as made: the 19 `fix` rows with the second opinion's sizing folded in (rows 23, 30 and 35 as
   **one synchronous validation prelude per executor overload**, refusing at the caller; row 42
   inside `SendAsync` only, no `finally` on overload 3; 22 before 63's third `[DataRow]`; 29 at all
   eleven doc sites; 53 with the positive arming proof; 38 with the full type name); **row 37p
   fixed** — an accepted proposal, `ContentNullAfterDeserializationException` public, the manifest
   moving by one row (measured 122 against the committed 121; the snapshot bot commits it on the PR
   head); **rows 6c, 18 and 54 declined and stated** (no primary handler of the package's own — one
   owned criterion replaces 6a/6b/6c, the cookie jar's scope one plugin; 18 a `LIFE`-owned ledger
   line with the `:227` message corrected as hygiene; 54 on the attribute's own criterion and
   `AC-PLUG-005.9`'s cost); **row 28n a proposal, declined for now and stated** — two bounds, two
   classes, a ledger line carrying the ask; parks 45 (the blocker named as the exception class on
   the body path), 64a and 64b with the consumer's note. The second opinion's corrections accepted:
   the band is 294 ms wider than `LogicBlockBase.MaxScheduledDelay` (one origin, not one number — the
   refusal is written to the package's own constant), 6c's scope is one container, row 18 loses the
   outcome and misnamed its cause, the executor had four `try` blocks not three, the DNS/socket
   entry is the handler's guarantee. The session's own findings beyond the classification are
   accepted: two criteria reworded to what their tests assert (`003.1`, `011.2`), three mutations
   that survived their first run made red by fixing the test (M3, M7a/b, M13), the arming proof
   pasted in both halves.
2. **Rounds.** Round 1 — a completeness critic (4 misses: a body the serializer refuses throwing out
   of the member synchronously — the executor's prelude read, the client's serialize not; a second
   registration's remove-then-add erasing a caller's User-Agent override; the timeout relabel catching
   any failure after the bound, not only a cancellation; the accepted GET-with-body clause not
   landed — plus `005.4` unreachable through the deferred dispatcher, 0 unmapped rows, 3
   citation-text mismatches of 59 read, the stale "exactly `MaxScheduledDelay`" paragraph, four
   stale ledger refs and the archive not the last touch) and an adversarial review (1 blocker: the
   archived doc's containment checkpoint claiming an archive undone and re-run that the history does
   not show; 6 conventions: the ledger refs, a composed CI-shape count, a culture Arrange that cannot
   fail, a magic number in a test name, the reverse question's `try` count, the journal's mutation
   count; 3 judgments: the User-Agent erase, the refusals undocumented on the interface,
   `SendRequest` with no callback leaking the response; 2 nits) → twelve items, worked by a fresh
   session in the sibling worktree `C:\_gh\dale-sdk-http-fixup` (skill v8) in twelve commits
   (`3dbea4d` … `2dd019f`); three brief premises refuted at the call site — `005.4`'s mutation does not
   fault the task (the core's own `catch` routes the escaped exception to the error callback, which
   is the observable), making `HttpClientName` public survives (the type is internal; the sharper
   mutation puts the name on a published type), and a request without a URI draws `ArgumentException`,
   not `ArgumentNullException`. The coordinator's targeted reads of every item at its call site found
   nothing further, so no second check ran.
3. **Accepted beyond the amendment's letter:** item 2 changes an exception class a callback receives
   today — a `JsonException` raised after the per-request bound arrived as
   `TimeoutException("Timed out after 0 seconds")` and now arrives as itself — classified a `fix` as
   the narrowing of a misreport on park 45's own path, and flagged; item 1's edge stated — a caller
   who sets exactly `HttpClient`'s own 100 s is indistinguishable from one who set nothing, so a
   second registration still applies 30 s there; the CI-shape count re-run and pasted (144 both
   ways) where the Phase B REPORT had typed 133. **Page: 36 criteria, 0 GAP; 144 test cases from 66
   test methods in 8 files, 66 `AC-HTTP` citations over 36 ids (35 in the suite, `013.2` from the
   analyzer-wiring probe); 77 rows; 40 delta lines, 4 `MODIFIED`.**
4. **Open for the operator, not decided:** row 28n — normalising the 30 s ceiling's
   `TaskCanceledException` to `TimeoutException` (the second opinion recommends it; the ceiling's
   message would lose its number unless the executor reads `HttpClient.Timeout`) — say "fix 28n";
   row 54 — `[assembly: DaleSharedAssembly]` (recommended against) — say "fix 54"; the parks with
   their owners (45 and 64a/64b — `HTTP`, the consumer named; 18 — `LIFE`); the `spec-pass` skill's
   retirement after this merge, a separate docs PR.
5. **Seen and left:** the manifest row is not on the branch — the snapshot bot commits it on the PR
   head (`releasing.md:112-115`), so the PR's second commit is expected; the archive commit is not the
   last touch on the page (`9a034cc` reworded two criteria after `3323894`, round 2 reworded four and
   minted one) — the containment proof is the coordinator's archive-gate run on a slug-renamed copy,
   pasted in the PR body; one premise test kept deliberately (the band against the cancellation
   source's own acceptance); a 3xx is not followed through the mocked seam and a lying
   `Content-Length` passes it — both named as harness limits on the page.
...
`[assembly: DaleSharedAssembly]`, and **66 test methods / 144 cases, every one citing an id**
(`grep -rho "\[TestMethod\]" Vion.Dale.Sdk.Http.Test/*.cs | wc -l` → 66;
`grep -rho 'TestProperty("spec", "AC-HTTP' Vion.Dale.Sdk.Http.Test/ | wc -l` → 66;
`dotnet test Vion.Dale.Sdk.Http.Test --no-restore` → `Passed! - Failed: 0, Passed: 144, Skipped: 0,
Total: 144`). The executor's **four** `try` blocks became **five** — the fourth was
`TryInvokeCallback`'s, and the fifth is the dispose-and-rethrow inside `SendAsync` that row 42's fix
added. *(This paragraph was measured at the pass's REPORT and re-measured by the fix-up round, which
added seven test methods and ten cases and left the type and member counts alone — the
`<exception cref>` tags of item 8 are documentation, not surface.)*
...
- **The out-of-range `timeout` threshold is ~49.71 days, not ~24.86.** The brief says "any delay over
  ...
  `TimeSpan.MinValue` throw. **This makes row 30 sharper, not weaker:** `4294967.294` seconds
  (`uint.MaxValue - 1` ms) has the **same origin** as `LogicBlockBase.MaxScheduledDelay`
  (`Vion.Dale.Sdk/Core/LogicBlockBase.cs:44`) — `uint.MaxValue` milliseconds — and is **not the same
  number**: `MaxScheduledDelay` is 4294967.000 s, the same milliseconds truncated to whole seconds,
  294 ms less. It is also `private static readonly`, so this package cannot reference it. Neither is
  derived from the other; the bound `AC-LIFE-006.2`'s refusal enforces one layer down comes from the
  same place, for the same reason. *(Corrected in amendment 1, second opinion § 2.1 / P1 — this
  paragraph kept the word "exactly" a round longer than row 30 and the page did, and amendment 2
  corrected it.)*
...
**The *Test today* column and the *Unmapped tests* list below name the suite as Phase A found it**,
before `T-003` rewrote it — they are the extraction's snapshot, not a pointer into today's files, and
the eight names they carry (`RegisterHttpClientRelatedServices`, `ConfigureHttpClientWithDefaults`,
`ThrowExceptionWhenJsonDeserializesToNull`, `SendRequestWithCorrectUri`,
`SendRequestWithCorrectContent`, `InvokeErrorCallbackWithExceptionWhenRequestTimesOut`,
`NotInvokeDispatcherWhenSuccessCallbackIsNull`, and `SendRequestWithCorrectHttpMethod`) resolve
against no declaration today by design. Every other `Class.Method` token in this doc was resolved
against a declaration by a script after the fix-up round's two renames: 2 resolved, 8 historical,
0 stale.
...
- **The User-Agent survives a double registration.** Two `AddDaleHttpSdk()` calls sent
  ...
  own `configureClient` still overrides it — and, since the fix-up round, so does a caller's
  `configureClient` on an **earlier** registration: each default is applied only where the client
  still lacks it, so a second `AddDaleHttpSdk()` no longer runs `Vion-DALE` and the 30 s timeout over
  a value the first registration's caller had chosen. The last `configureClient` still wins for
  whatever it sets itself.
- **A failure raised after a per-request timeout elapsed keeps its own class.** Once the bound had
  fired, every failure was reported as `TimeoutException("Timed out after n seconds")` — a body that
  would not parse, a broken stream, a 502 judged late. The body read and the deserialization run
  outside the token, so that window is real. Those now arrive as `JsonException`, as whatever the
  stream raised, and as `HttpRequestException`; a genuine expiry of the bound is unchanged.
- **`SendRequest` with no success callback no longer leaks its response.** The overload has no
  `finally` because a callback owns the response it is handed; with no callback there was no owner
  and the response was dropped with its body stream open. It is now disposed in that branch only —
  a callback's response is still the callback's, still undisposed, still readable after the hop.
- **The refusals are documented.** All eight `ILogicBlockHttpClient` members and the three executor
  overloads carry `<exception cref>` tags for what they throw synchronously: a missing dispatcher, a
  missing `SendRequest` request or `RequestUri`, a timeout outside the accepted band, and — on the
  four members that take a body — the serializer's own exception, which has always escaped at the
  caller and is now stated as `AC-HTTP-007.4`. The shipped XML documentation file changes; no
  signature does.
- **The log message a support engineer greps for changed.** `LogCallbackFailed` read "actor may be
  disposed", which named the wrong cause: the dispatcher refuses the self-send while the block has
  not yet received its first message, and that is the common case. It now says so
  ("it may not have received its first message yet, or may already have stopped"). Log text is not a
  contract (`testing-conventions.md` § 15), but it is the string that gets searched.
...
- 2026-09-06 (self-check, item 2): reading each criterion's **text** against its assertions found two
  ...
  overload stops disposing its request → red).
- 2026-09-06 (amendment 2, review finding 1 — the blocker): **the checkpoint above claimed "the
  archive was undone and re-run for it, so the archive commit stays the last touch on the page and
  the delta". The history says otherwise, and it is corrected here rather than left standing.**
  `3323894` is the archive commit and is a rename and nothing else (`git show --stat 3323894` →
  `docs/changes/{ => archive}/2026-09-06-http-pass.md | 2 +-`). `9a034cc`, three commits later,
  edited the archived doc **in place** and `docs/specs/http.md` in the same commit, with no
  un-archive/re-archive rename; `26638fe` then edited the archived doc's counts. So two criteria
  (`AC-HTTP-003.1`, `AC-HTTP-011.2`) were reworded after archival and skill v11's containment did
  not hold. The rewordings themselves stand — page text and delta line are identical and the map
  rows at *Consolidation map* say why.
  This fix-up round edits both files again by design: `AC-HTTP-001.2`, `004.1`, `004.2` and `006.1`
  are reworded on the page and carry `MODIFIED` delta lines, `AC-HTTP-007.4` is minted and carries
  an `ADDED` line, and the prose beside each changed with it. The containment proof for this round
  is therefore **the coordinator's close-out**, which re-runs `spec-change.ps1 archive` on a
  slug-renamed copy placed back under `docs/changes/` — the archived file cannot be archived twice
  without doubling its relative links. This round deliberately does not run that step.
...
- MODIFIED AC-HTTP-004.1 -> docs/specs/http.md : WHEN a caller uses `SendRequest` THE SYSTEM SHALL send the `HttpRequestMessage` it is given, taking no URL and no headers of its own, including a body on any method and the caller's own content type.

---

## Consolidation map

> Row → criterion, or row → the line saying why it mints nothing. All **77** rows appear. **34
> criteria come from rows**; the page carries **36** in all (``grep -c '^- .AC-HTTP-' docs/specs/http.md``
> → 36), because two mint from findings no Phase A row carried: `AC-HTTP-010.1` from the second
> opinion's unrowed observable 3, and `AC-HTTP-007.4` from the completeness critic's M-1. The sweeps
> over-produced on purpose and the page states one criterion per *rule*, with the members, failures
> and edge values the rule ranges over as that criterion's `[DataRow]`s.

| Rows | Criterion |
| --- | --- |
| 1, 2, 8 | `AC-HTTP-001.1` |
| 3, 4 | `AC-HTTP-001.2` — reworded by the fix-up round: the "a caller's own timeout or `User-Agent` wins" clause was false across two registrations, so the criterion now says a later registration does not undo it (critic M-2 / review 8) |
| 5 | `AC-HTTP-001.3` |
| 69 | `AC-HTTP-001.4` |
...
| 12, 16 | `AC-HTTP-003.2` |
| 13, 22 | `AC-HTTP-003.3` |
| 14, 51 | `AC-HTTP-004.1` — reworded by the fix-up round to carry amendment 1's accepted item 5 (P7): a body on any method and the caller's own content type |
| 43 | `AC-HTTP-004.2` — reworded by the fix-up round: the overload disposes the response itself where the caller gave no callback to own it (review 10) |
...
| 21, 72 | `AC-HTTP-005.3` |
| 20 | `AC-HTTP-005.4` |
| 25, 33, 34, 36, 37, 38, 39, 70, 71 | `AC-HTTP-006.1` — reworded by the fix-up round so that a per-request timeout is reported as one only where that bound cancelled the exchange (critic M-3, on park 45's own path); one `[DataRow]` per failure; 37's `<exception>` tag becomes correct through 37p, and 38's wording is asserted on the null-body row |
| 24 | `AC-HTTP-006.2` |
| 23 | `AC-HTTP-007.1` |
| 30 | `AC-HTTP-007.2` |
| 35 | `AC-HTTP-007.3` |
| M-1 | `AC-HTTP-007.4` — the fix-up round's own row: the serializer refuses a body at the caller. It is the fourth synchronous throw, and Phase A's statement sweep read the executor's prelude without reading the client's serialize beside it (the symmetric-side miss the completeness critic found) |
...
- the 5 `try` blocks and the `using` outside each: row 30 for the `using`; the three overloads' own
  (`HttpRequestExecutor.cs:135`, `:177`, `:214`) for rows 42 and 44, `TryInvokeCallback`'s (`:345`)
  for rows 18, 20 and 23, and the dispose-and-rethrow inside `SendAsync` (`:299`) that row 42's fix
  added. *(Phase A counted three, amendment 1 found `TryInvokeCallback`'s fourth, row 42's fix made
  five, and this line was corrected in amendment 2.)*
...
**One test in the suite pins an implementation premise, deliberately.**
`HttpRequestExecutorShould.RefuseExactlyWhatCancellationSourceRefuses` asserts that
`CancellationTokenSource` takes `HttpRequestExecutor.MaxRequestTimeout` and refuses one millisecond
more — the platform's own band, not this package's rule. It is kept because every other band test
expresses its rows relative to the package's constant and would follow that constant anywhere it
moved (M13's lesson); this is the one assertion that would notice a constant drifting off the
runtime's actual bound. It carries `AC-HTTP-007.2` rather than staying uncited, because the
criterion's subject *is* "the timeout the runtime's cancellation source will not accept".
*(The Phase B REPORT said "nothing in this suite pins an implementation premise"; the critic found
this one, and amendment 2 records it here, where the REPORT's sentence is not durable.)*
...
## Scorecard (pass 14 — attempt 1, one classification relay, one amendment as a fresh-session brief, zero reverts)

| Measure | Value |
|---|---|
| Gates (build/test/lint/trace/style/doc-comment/bom/journal/residue/self-tests/cleanup/archive/CI shape) | green — every line a paste from the round-2 REPORT on `spec-pass/http-a1-fixup` at `2dd019f`: `dotnet build Vion.Dale.Sdk.sln` **0 warnings / 0 errors** and a full `--no-incremental` build with **0 DALE diagnostics**; `dotnet test Vion.Dale.Sdk.sln` 30 suites, all passed, 0 failed; the HTTP suite `Passed: 144`, identical plain and under the CI shape (`-p:Version=0.0.0-ci.1`); `spec-lint` plain and `-Diff main` OK; `spec-trace` OK — 1105 ids, 14 traced pages, no HTTP id a GAP; `test-style-lint` 2527 cited, **0 exempt files**; `doc-comment-lint` 3193 in 946; `bom-lint` 165; `journal-lint` 266; `sweep-residue-lint` 1014; `run-script-tests` 9 passed; the arming proof in both halves (`DALE014` on the probe type; 0 on the shipped tree); `cleanup-code -Changed -NoBuild -Verify` `Code style: clean.` on a clean tree; no Tier 2 and no smoke tier owed (nothing reaches the DevHost); the coordinator's re-run on the close-out worktree and the archive gate on a renamed copy are in the PR body; CI on the PR. |
| Completeness-critic misses | **4** — symmetric-side 1 (the client's synchronous serialize beside the executor's synchronous prelude), state-interaction 2 (a second registration × a caller's `configureClient`; a failure after the per-request bound × the relabel predicate), consumer/statement 1 (the accepted GET-with-body clause not landed) — plus `005.4` unreachable through the deferred dispatcher (v12), **0 unmapped classified rows**, 3 citation-text mismatches of 59 read (a culture claim the platform gives for free; "and send it" unsent; "no public member exposes" unasserted), the stale "exactly" paragraph, four stale ledger refs and the archive order, all resolved in round 2. |
| Evidence errors found in review | round 1: **1 blocker** (the archived doc's containment checkpoint claiming an archive undone and re-run that the history does not show), **6 conventions** (the ledger refs read off the pre-fix file; a CI-shape count typed as 133 against 134 both ways; a culture Arrange whose deletion changes nothing; a magic number in a test name; the reverse question's `try` count; the journal's "two of seven" against 24 mutations), **3 judgments** (the User-Agent erase; the refusals undocumented on the shipped interface; `SendRequest` with no callback leaking the response), **2 nits**; the **brief's** errors: 11 claims and 10 omissions corrected by a fresh-context check before dispatch, the second opinion's 294 ms correction of the pass's arithmetic, and three round-2 brief premises refuted by the fix-up at the call site (the `005.4` mutation, the `HttpClientName` mutation, the exception type for a missing URI). Round 2 (targeted reads): **0**. |
| Mutation evidence | complete — 24 mutations run at Phase B (M1–M24), three surviving their first run and made red by fixing the test rather than weakening the mutation (M3 the fixture's handler ignoring the token; M7a/b `Headers.Add` on a disposed request; M13 rows relative to the constant); round 2's ten new or changed tests each with its mutation pasted red then green (the remove-then-add restored; the state-only relabel restored; the serializer's refusal routed to the callback; the else-dispose dropped and the counter-mutation `finally` reddening the ownership test; a GET's content stripped; the catch removed; the bound sentence dropped; a null body refused; the name on a published type; the timeout guard alone), one premise test kept deliberately. |
| Operator corrections (table + PR) | at the classification gate: **0 — delegated**; the coordinator classified in the operator's stead with no override of the second opinion (37p accepted; 6c, 18, 54 declined and stated; 28n left a proposal and flagged; the sizing of 23/30/35 and 42 folded in); coordinator rounds: round 1 (critic 4 misses · 1 reachability · 0 unmapped · 3 citations; review 1 blocker · 6 conventions · 3 judgments · 2 nits), round 2 (targeted reads: 0 findings; three brief premises refuted by the fix-up session at the call site). |
| Cost | 2 Opus sessions at high effort — the pass session (no stall), the fix-up session in a sibling worktree — · 1 classification relay + 1 amendment as a fresh brief, each delivered within a minute (both ends in bypass mode) · 5 Opus subagents for this pass (the scoping inventory and the brief check the evening before; the second opinion 24 min; the critic 9.8 min; the review 12.5 min) · 0 reverts · ~3 h 01 min wall from launch (08:05) to the round-2 REPORT (11:06): Phase A 21 min, the second opinion and classification 39 min, Phase B 66 min, the checks and the fix-up brief 21 min, round 2 34 min — in parallel with the consistency, findings and Jira reviews and the unification pass's first tier. |
