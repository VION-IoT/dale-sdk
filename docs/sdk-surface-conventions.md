...
- **`--exclude "*.Test"` is load-bearing, not hygiene.** `Vion.Dale.Sdk.Generators.Test` has five
  `[PublicApi]` types and would otherwise ship in the manifest. The six **TestKits** are not excluded —
...
- **The analyzer pack does not run over every SDK project.** **Nineteen** projects reference
  ...
  six TestKits, `Vion.Dale.Sdk.Test`, `Vion.Dale.Sdk.TestKit.Test`, `DevHost.Test`, `DevHost.SmokeHost`
