...
- **`[assembly: PublicApiNamespace]` is what puts an assembly in the manifest**, declared at the source
  in that assembly's own `PublicApiConfig.cs`. There is no curated list anywhere else. Twelve
  assemblies opt in today: `Vion.Dale.Sdk`, the five I/O and protocol contract assemblies (`AnalogIo`,
  `DigitalIo`, `Modbus.Core`, `Modbus.Rtu`, `Modbus.Tcp`), `Http`, and the five TestKits, which opt in
  deliberately. CI's `--exclude "*.Test"` drops the **unit-test** projects, not the TestKits.
- **Marking a type `[PublicApi]` in an assembly that never opts in does nothing.** `Vion.Dale.DevHost`,
  `Vion.Dale.Cli`, `Vion.Dale.Plugin`, `Vion.Dale.ProtoActor` and `Vion.Dale.LogicBlockParser` declare
  no `PublicApiNamespace`, so no change to them can move the manifest. A plan that predicts snapshot
  churn from touching one of those is wrong — check the assembly's `PublicApiConfig.cs` before
  believing it. (`IDevHostControl` has been described in briefs as `[PublicApi]`; it carries no such
  attribute and appears in no snapshot.)
