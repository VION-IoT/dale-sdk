...
5. Wire and consumer surfaces: <the area's files, schemas, HTTP or CLI surfaces> changed exactly
   ...
   this branch's fragment, and record rather than prescribe; every
...
| `VION-IoT/shared-workflows/actions/journal-lint` | `spec-gates.yml`'s `journal-lint` job + on demand, from a shared-workflows checkout | an entry in `docs/process-journal/`'s live window - every fragment in the folder - off the grammar its `README.md` states; the action owns the rules |
