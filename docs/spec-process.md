...
| `scripts/run-script-tests.ps1` | `spec-gates.yml` (on a pull request, when a script, that workflow or a project file changed) + on demand | any `scripts/*.tests.ps1` self-test fails, or a gate script has neither a self-test nor an exemption-with-reason |
