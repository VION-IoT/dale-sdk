...
| `VION-IoT/shared-workflows/actions/journal-lint` | `spec-gates.yml`'s `journal-lint` job + on demand, from a shared-workflows checkout | an entry in `docs/process-journal.md`'s live window off the grammar its header states; the action owns the rules |
