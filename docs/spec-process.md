...
`scripts/bom-lint.ps1`, `scripts/journal-lint.ps1`, `scripts/sweep-residue-lint.ps1`,
`scripts/run-script-tests.ps1`, and `/cleanup` once. Where a fixture asserts a build-time literal,
add a run under CI's shape ([`testing-conventions.md`](testing-conventions.md) § 8). Stryker.NET is
