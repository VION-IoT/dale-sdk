...
| `scripts/self-reference-lint.ps1` | `spec-gates.yml` + on demand | a Markdown table cell of at most 60 characters carrying `this PR`, `this commit`, `this branch` or `the current PR` — a pointer column filled with a phrase that only resolves before the merge, and is read after it. Cells only, so narrative saying "this PR touches `scripts/`" is untouched, and the length bound is what separates a pointer from a paragraph; the append-only logs, `docs/rfcs/`, `docs/changes/archive/` and the snapshots are out of scope, being written once and never revised. Two *Implementation state* rows shipped this way one commit after the journal line recording it |
