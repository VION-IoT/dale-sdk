...
- **R11 — Interface mappings to a gated-out endpoint are not skip-and-warned (found in impl, Brief B).** Contract mappings whose target contract is gated out are dropped with a warning at initialize (`LogicBlockBase` skip-and-warn, the tested `SkipAndWarnWhenAContractMappingTargetsAGatedOutContract` path). Interface mappings have **no equivalent guard**: a topology `interfaceMapping` to a gated-out interface (e.g. mapping a controller to a charge-point interface the count excludes) is accepted at load (validation is type-level), is listed in the exported `interfaceMappings`, and the sender links + sends to it — the receiver then throws `KeyNotFoundException` in `LogicBlockBase.GetFunctionByIdentifier` on every send. Divergence from the contract path (R4's "removed together" only holds for the endpoint, not for mappings *targeting* it). Fix candidate: skip an interface mapping whose resolved target interface is not bound, mirroring the contract skip-and-warn. Surfaced by the `Vion.Examples.Gating` fixture; the example works around it by only mapping to included points.
