...
**Deferred (real-time-fidelity & typed-value validation).** Three related items from the consumer field log are recorded but not built (none a quick win; the repo has issues disabled, so they live here):
- **DF-23** — a wall-clock counterpart to `ScenarioRunReport.WatchTrace` (sampled `watch[]` trace on a non-stepped host, or a REST live-values snapshot), so wall-clock loop-stability forensics don't fall back to DOM-scraping. This is the enabler that makes the wall-clock home for DF-24 ergonomic.
