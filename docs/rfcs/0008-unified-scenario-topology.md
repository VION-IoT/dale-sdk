...
1. **Quiescence reliability in Proto.Actor** *(highest risk)* — the spike must prove deterministic, deadlock/livelock-free settling. **Resolved + extended (DF-18).** The exact quiescence barrier settles correctly, but quiescence *detection* is not message-ordering *determinism*: on a **fan-in** network (independent timers due at the same virtual instant; many sources → one aggregator), the barrier waits for concurrent handlers but does not order them, so the cross-actor processing order was thread-pool-dependent and a real EnergyManager scenario converged in 6 vs 8 virtual-time hops run-to-run. Fixed by pinning **in-round ordering** in stepped mode: (a) a single shared serial dispatcher (`DeterministicDispatcher`) so a message cascade drains in one reproducible order instead of fanning across the thread pool, and (b) **stepper-driven delivery** — `SendToSelfAfter` registers the send in `IVirtualSchedule` and the next-event stepper delivers same-virtual-instant timers one at a time in `(due-time, registration)` order, rather than letting racing `Task.Delay` continuations deliver them. Both are gated to a `FakeTimeProvider` clock (zero effect on the free-running Player / production). Guarded by fan-in and fan-out determinism tests (`FanInDeterminismShould`).
...
### 11.7 Assert STATE, not the schedule

The single most important robustness rule: **a scenario asserts the system's STATE and invariants — never the simulation's internal schedule.** Hop counts, the number of `advance` cycles to converge, and the order events fire are properties of the *stepped simulation's scheduling*, not of your blocks' behaviour. In the real wall-clock runtime they are meaningless and genuinely non-deterministic; coupling a test to them is brittle (a timer-rate tweak, a reordered or added block, or a wiring change moves the hop count without changing the behaviour).

- **Drive to a steady state, then assert it.** `settle` (advance until the targeted values stop changing) followed by `expect` is the robust shape — converge, then check the converged STATE. `advance {seconds}` + `expect` works the same way when you know the budget (e.g. the Energy `peak-shaving` scenario: advance 40 s, then assert `Battery.ActivePowerDischarging == 5 ±0.05`). `waitUntil` asserts a *condition on state* (`Light.On == true`), which is exactly right — the wait is satisfied by the state, not by a hop count.
- **Never assert on a report `Detail` string or a hop count.** `Detail` (e.g. `"satisfied after 6 hops / 3.2 virtual s"`) is human-readable diagnostic prose, not a contract — its wording and numbers can change between SDK versions even when behaviour is identical. If you need a reproducible elapsed for a report diff, use the deterministic `ScenarioStepResult.VirtualElapsedMs` (stepped hosts), not `ElapsedMs` (wall clock) and not the `Detail` text.
- **Determinism is the SDK's guarantee — don't re-prove it per consumer.** A stepped host is bit-reproducible (serial in-round dispatch + stepper-driven, ordered delivery; guarded by the SDK's own fan-in/fan-out determinism tests). You do not need a "run it 5× and diff the trace" probe in your suite to trust that; assert your behaviour once. (Such a probe is a fine *diagnostic* if you ever suspect a regression in the substrate — but file it as an SDK bug, it is not a behaviour your scenario owns.)
- **Reach for struct field paths** (`Block.Member.Field`, e.g. `RefControllableConsumer.AllocatedCurrent.L1`) so a per-phase scalar assertion stays in JSON instead of graduating to C#; `waitUntil` / `expect` resolve them against the wired graph.
- **Graduate to C# only for what the vocabulary can't express** (`ScenarioRunner.ApplyAsync` runs setup + steps, then you add arbitrary typed asserts on `control.GetProperty(...)` / `GetDigitalOutput(...)`) — again, asserting STATE, not the schedule.

Rule of thumb: if an assertion would still be meaningful against the *production* (wall-clock) runtime, it is a behaviour worth testing; if it only makes sense because of how the stepped clock schedules events, it is testing the simulator, not the system.

**Fidelity boundary — stepping proves state/orchestration, not closed-loop real-time stability (DF-24).** The deterministic stepper drains to full quiescence (`QuiescenceBarrier`) before each control cycle, so a control loop always reads *fully-settled* feedback — it collapses the command→feedback round-trip to instant. That is exactly what makes stepping deterministic and instant, but it means a closed loop that **limit-cycles in real time** — because its command→feedback round-trip exceeds the control period, so the controller acts on stale feedback — **settles cleanly on the stepped gate.** Confirmed in the field: the same EM code and topology at a 1 s control interval settles to −1.9 kW stepped but oscillates 0 ↔ −3.8 kW in the free-running Player (and the oscillation disappears at a 3 s interval). So "green on the stepped gate" proves logical/state correctness, **not** real-time loop stability. A "no sustained oscillation" property is a *wall-clock* question — characterise it on a wall-clock lane (a non-stepped `HeadlessHost.Load(topology)` plus a sampled poll), not on the deterministic gate. This is the boundary of the §11.7 posture: assert *state* on the gate; characterise *loop stability* in wall-clock.

**Anchor time-relative `set` data to the deterministic epoch (DF-25).** A stepped host's `FakeTimeProvider` starts at a fixed epoch (`2026-01-01T00:00:00Z`, `DevHostBuilder.WithDeterministicStepping`). A scenario that `set`s a value carrying timestamps — e.g. a schedule/plan whose entries are compared against the clock — must anchor those timestamps to the epoch: an "already-active" entry sits at the epoch (and within the block's plan-entry max-age — an ancient literal like `2000-01-01` reads as *expired*, not active); a "future" entry is `epoch + offset`, reached with an `advance` step. Literal wall-clock dates do not line up with the virtual clock.

**Deferred (real-time-fidelity & typed-value validation).** Three related items from the consumer field log are recorded but not built (none a quick win). Consumer SDK feedback is tracked in Jira under the "Dale SDK Feedback" epic (VION-62) since 2026-08; these three predate it and stay recorded here until filed:
- **DF-23** — a wall-clock counterpart to `ScenarioRunReport.WatchTrace`: samples on a non-stepped host carry no time coordinate (`WatchSample.VirtualElapsedMs` is null there) and there is no periodic sampling during a real-clock `advance`/`settle`, so wall-clock loop-stability forensics still fall back to hand-rolled pollers. (The REST live-values half shipped: `GET /api/state/{block}` / `/{block}/{prop}`.) This is the enabler that makes the wall-clock home for DF-24 ergonomic.
- **DF-24(b)** — an opt-in *bounded-settle / real-time-budget* stepping mode that drains the quiescence barrier only up to the control period, so a real loop limit-cycle surfaces deterministically on the gate. Genuinely hard (it requires virtual-time message latencies + firing the control timer without full quiescence — a discrete-event real-time simulator whose fidelity is only as good as a synthetic latency model); the wall-clock path above is the preferred direction. Demand signal: AC-RGM-004.1's meter-lag oscillation proof has no home on the deterministic gate today.
- **DF-25 sub-1** — type the `set`/`setup` **value** against the target property's struct shape in `dale scenario schema` enrichment + `ScenarioFileChecks` (today `value` is untyped), so a mis-cased key on a struct / array-of-struct `set` is caught at validate time instead of binding silently to an empty struct. The shared `Vion.Contracts.PropertyValueCodec` is deliberately *not* in scope — tolerant struct decoding is platform-wide forward-compat behaviour.
