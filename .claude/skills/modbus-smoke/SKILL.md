...
- `DebugClient` → **connection** renders the two editable structs, *Connection* and *Link policy*, each
  ...
  **Diagnostics** viewer the SDK's own summaries are labelled too (`Link state`; the round-trip
  maximum is scaled to `910 ms` rather than `PT0.91S`) — all of it since 0.10.5. An enum-typed struct field
...
through `ScenarioRunner`, so `dotnet test Vion.Dale.Sdk.sln` guards the link policy on every push `publish.yml` builds —
