...
(4) drive + assert with `advance` / `settle` / `waitUntil` / `expect`; (5) run it, read the failure `detail`, adjust. Three
...
its own `waitUntil` (the fields of one struct member arrive together; `dale scenario validate` warns on
this only after a `serviceProviderSet`, never after a `set`), and keep `expect` for a value the drive leaves alone, where a wait would pass on the old
